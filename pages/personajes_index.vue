...
<template>
  <div class="container">
   <HeaderView />
   <h3 style="margin-top: 15px">Characters </h3>
   <p id=parra>This category lists characters. </p>
   <ul class="ul">
     <li v-for="p of personajes" :key="p.slug">
       <div class="card">
          <div>
            <img class="card-img" :src="'/images/personajes/' + p.image" />
          </div>
          <div class="card-text">
            <h4 class="card-title"><strong> {{ p.nombre }} </strong></h4>
            <p class="card-description">
              <span>Age: {{p.edad}}</span><br>
              <span>Born house: {{p.casa_nacimiento}}</span><br>
            </p>
